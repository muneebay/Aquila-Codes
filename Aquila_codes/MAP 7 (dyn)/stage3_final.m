...
%% Creating node and topic
 node = ros2node('MATLAB'); %creates a ROS 2 node named 'MATLAB' and establishing a connection between MATLAB and ROS.
 velocityPub = ros2publisher(node, "/position", "geometry_msgs/Point"); %creates a publisher object named velocityPub that publishes messages of type "geometry_msgs/Point" to the topic "/position"
 msg = ros2message(velocityPub); % creates message of the same type as publisher, and it is used to store and send position information.
%%
...
maxrangeClsion = 2; %maxrange to detect collition
 ...
 map = robotics.OccupancyGrid(bound,bound,20); %resolution is the number of pixels in one cell; no needs to consider it below
 show (map);
...
k_(1:3) = 15;
k__ = [k;k_];
obs_10 = transpose(k__);
setOccupancy(map, obs_10, 1)

l = 4.7:0.1:4.9;
l_(1:3) = 15;
l__ = [l;l_];
obs_11 = transpose(l__);
...
m_(1:3) = 6;
m__ = [m;m_];
obs_12 = transpose(m__);
setOccupancy(map, obs_12, 1)
...
n_(1:3) = 6;
n__ = [n;n_];
obs_13 = transpose(n__);
setOccupancy(map, obs_13, 1) %bottom right
...
% try 
 ...
 map = robotics.OccupancyGrid(bound,bound,20); % map is defeined after every loop
cntx = bound/2;
cnty = bound/2;
s = cnty/0.1;
A(1,s) = cntx-1;
