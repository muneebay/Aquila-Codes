...
res=10;
 map = robotics.OccupancyGrid(bound,bound,res); %resolution is the number of pixels in one cell; no needs to consider it below
 show (map);
...
maxrange = 5; %beacon length jha pa search kr rha hota h
...
%             robots(j).Itertn(i).Pose = [3,5];
            robots(j).Itertn(i).Pose = [10,10,0];
        end
        if j==2
%             robots(j).Itertn(i).Pose = [5,4];
             robots(j).Itertn(i).Pose = [10,13,0];
        end
        if j==3
...
%% DYnamic Obstacles
 map = robotics.OccupancyGrid(bound,bound,20); % Creating an occupancy grid map
    for i=1:nIter % i is starting from to no. iterations
        
if(i==1) %phli iteration ka code diff h,  q k us ma wo move nhi kr rha hota
cntx = bound/2;
cnty = bound/2;
s = cnty/0.1;
A(1,s) = cntx-1;
...
for jj=1:1:180 % obstacle ko define krnay k liye, version dimension (length)
    pp=1+(jj-1)*180;
for kk=pp:1:pp+180 %horizontal dimension
    k__(kk,1)= 13.7-0.1*i+(kk-(jj-1)*180-1)*0.01; % x-axis obstacles
    k__(kk,2)=13.7+0.01*(jj-1);% y-axis obstacle
    
    l__(kk,1)= 6+(kk-(jj-1)*180-1)*0.01;
    l__(kk,2)=13.7-0.1*i+0.01*(jj-1);
    
    m__(kk,1)= 6+0.1*i+(kk-(jj-1)*180-1)*0.01;
    m__(kk,2)=6+0.01*(jj-1);
    
    
    n__(kk,1)= 13.7-0.1*i+(kk-(jj-1)*180-1)*0.01;
    n__(kk,2)=6+0.1*i+0.01*(jj-1);

end
end
% obstacle
setOccupancy(map, k__, 1)
setOccupancy(map, l__, 1)
setOccupancy(map, m__, 1)
setOccupancy(map, n__, 1) %bottom right



% unexplored
else % uper wala code i=1 k liya h 
setOccupancy(map, k__, 0.5)
setOccupancy(map, l__, 0.5)
setOccupancy(map, m__, 0.5)
 setOccupancy(map, n__, 0.5) %bottom right
...
for jj=1:1:180
    pp=1+(jj-1)*180;
for kk=pp:1:pp+180
    k__(kk,1)= 13.7-0.1*i+(kk-(jj-1)*180-1)*0.01;
k__(kk,2)=13.7+0.01*(jj-1);
...
setOccupancy(map, n__, 1) %bottom right
               ...
               msg.x =  linearTransform(robots(1).Itertn(i).Pose(1),0,20,-2.10,2.3);
               % 
               msg.y =  linearTransform(robots(1).Itertn(i).Pose(2),0,20,-2.25,2.2);
            ...
            pause(0) %wait for robot to reach destination
        val = 1;
        for j=1:nRbt       
    %         if i ~= 1
                x = robots(j).Itertn(i).Pose(1,1);
