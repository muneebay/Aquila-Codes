...
thresholdClsionDttn = 0.9; %threshold of collision detection
maxrangeClsion = 2; %maxrange to detect collision

% Create a figure
figure;

% Define the dimensions of the 3D map
x = 0:0.1:bound;
y = 0:0.1:bound;
...
% Create the Z matrix for the 3D map
Z = zeros(size(X));
...
% Set occupancy of obstacles in the map
for i = 1:size(obs_10, 1)
    x_idx = round(obs_10(i, 1) * 10) + 1;
    y_idx = round(obs_10(i, 2) * 10) + 1;
    Z(y_idx, x_idx) = 1; % Swap x and y indices to match the meshgrid dimensions
end

for i = 1:size(obs_11, 1)
    x_idx = round(obs_11(i, 1) * 10) + 1;
    y_idx = round(obs_11(i, 2) * 10) + 1;
    Z(y_idx, x_idx) = 1;
end

for i = 1:size(obs_12, 1)
    x_idx = round(obs_12(i, 1) * 10) + 1;
    y_idx = round(obs_12(i, 2) * 10) + 1;
    Z(y_idx, x_idx) = 1;
end

for i = 1:size(obs_13, 1)
    x_idx = round(obs_13(i, 1) * 10) + 1;
    y_idx = round(obs_13(i, 2) * 10) + 1;
    Z(y_idx, x_idx) = 1;
end

% Plot the 3D map using surf
surf(X, Y, Z);
hold on;

% Create contour lines for obstacles
contour3(X, Y, Z, [0.5 0.5], 'LineColor', 'r');
% hold off;
